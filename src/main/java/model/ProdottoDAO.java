...
public class ProdottoDAO {
 ...
 // Metodo helper 
    ...
    // Recupera tutti i prodotti visibili presenti nel Database
    public synchronized List<ProdottoBean> doRetrieveAll(boolean onlyVisible) throws SQLException {
        List<ProdottoBean> prodotti = new LinkedList<>();
 ...
 //  Recupera i prodotti appartenenti ad una specifica categoria
    public synchronized List<ProdottoBean> doRetrieveByCategoria(long idCategoria, boolean onlyVisible) throws SQLException {
        List<ProdottoBean> prodotti = new LinkedList<>();

        String selectSQL = "SELECT p.*, c.nome AS nomeCategoria " +
                           "FROM " + TABLE_NAME + " p " +
                           "LEFT JOIN categoria c ON p.idCategoria = c.id " +
                           "WHERE p.idCategoria = ?" + (onlyVisible ? " AND p.visibile = 1" : "");
